import { FigmaNode, FigmaApiResponse } from './types'
...
export async function fetchFigmaNode(fileKey: string, nodeId: string): Promise<FigmaNode> {
  ...
  try {
    ...
    throw new Error(
      `Network error fetching Figma API: ${(networkError as Error).message}\n` +
  ...
  if (response.status === 404) {
    throw new Error(
      `Figma API returned 404 Not Found for file key "${fileKey}".\n` +
      'Check that:\n' +
  ...
  let data: FigmaApiResponse
  try {
    data = (await response.json()) as FigmaApiResponse
  } catch {
  ...
  if (!data.nodes || Object.keys(data.nodes).length === 0) {
    throw new Error(
      `No nodes found in Figma API response for node ID "${nodeId}".\n` +
  ...
  if (!nodeEntry || !nodeEntry.document) {
    throw new Error(
      `Node "${nodeId}" not found in Figma API response.\n` +
