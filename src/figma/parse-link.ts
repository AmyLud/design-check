...
export function parseFigmaLink(link: string): { fileKey: string; nodeId: string } {
  ...
  const designOrFileIndex = pathSegments.findIndex(
    (seg) => seg === 'design' || seg === 'file'
  )

  if (designOrFileIndex === -1 || designOrFileIndex + 1 >= pathSegments.length) {
    throw new Error(
      `Could not extract file key from Figma URL path: "${url.pathname}"\n` +
  ...
  if (!rawNodeId) {
    throw new Error(
      `Missing node-id query parameter in Figma URL: "${link}"\n` +
...
export function validateFigmaLink(link: string): boolean {
  ...
  const pathSegments = url.pathname.split('/').filter(Boolean)
  const hasSupportedSegment = pathSegments.some(
    (seg) => seg === 'design' || seg === 'file'
  )
  if (!hasSupportedSegment) return false

  // Must have a node-id query param
  if (!url.searchParams.has('node-id')) return false
